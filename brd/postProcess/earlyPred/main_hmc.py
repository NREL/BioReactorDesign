...
def SREModel(theta, x):
    ...
    F = (
        ...
        * (
            0.5
            - 1
            / (1 + jnp.exp(jnp.clip((a * x) ** b, a_min=None, a_max=maxExp)))
        )
    )
    F += (
        2
        ...
        * (
            0.5
            - 1
            / (1 + jnp.exp(jnp.clip((c * x) ** d, a_min=None, a_max=maxExp)))
        )
    )
    return F
...
for idat, datf in enumerate(data_files):
    ...
    plt.plot(
        ...
        markersize=5,
        linewidth=3,
        label=f"{datf} y={med_real[-1]:.2f}+/-{(std90_real[-1]-std10_real[-1])/2:.2f}",
    )
