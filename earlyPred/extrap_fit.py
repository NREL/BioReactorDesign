...
def plotAll(data_files, data_arr, color_files=None, chop=False, extrap=False):
    ...
    for idat, datf in enumerate(data_files): 
        if color_files is not None:
            ...
            color=None
        if extrap:
            plt.plot(data_arr[datf]["textrap"], 100*data_arr[datf]["yextrap"],'--', color=color)
        if chop:
            lim = data_arr[datf]["lim"]
            plt.plot(data_arr[datf]["t"][lim:], 100*data_arr[datf]["y"][lim:], 'o', color=color)
        else:
            plt.plot(data_arr[datf]["t"], 100*data_arr[datf]["y"], color=color)
...
maxExp = 20 
def f(x, a, b, c, d, e, f):
    return 2*c * (0.5 - 1/(1+np.exp(np.clip((a*x)**b, a_min=None, a_max=20)))) + 2*f * (0.5 - 1/(1+np.exp(np.clip((d*x)**e, a_min=None, a_max=20))))
...
for idat, datf in enumerate(data_files):
    ...
    popt, pcov = curve_fit(f, t_fit, y_fit, bounds=([0, 0, 0, 0, 0, 0], [0.05, 1.7, np.inf, 0.015, 1.3, np.inf]))
