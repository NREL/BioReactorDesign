...
import os
import shutil
from bird.preprocess.species_gen.setup_thermo_prop import write_species_properties, get_species_name, get_species_properties, get_species_key_pair
...
def test_species_thermo_write():
    ...
    src_fake_thermo = os.path.join(case_dir, "constant", "thermophysicalProperties_erroneous.gas")
    dest_thermo = os.path.join(case_dir, "constant", "thermophysicalProperties.gas")
    shutil.copyfile(src_fake_thermo, dest_thermo) 
    
    foam_dict = read_openfoam_dict(dest_thermo)
    assert abs(float(foam_dict["O2"]["thermodynamics"]["highCpCoeffs"][2])+7.57967e-07) > 1e-12
...
def test_species_prop():
    ...
    assert abs(float(species_prop["O2"]["gas"]["transport"]["As"]) - 1.948e-06) < 1e-12
    ...
    assert abs(float(species_prop["O2"]["gas"]["transport"]["As"]) - 1.948e-06) < 1e-12
    assert abs(float(species_prop["N2"]["gas"]["thermodynamics"]["lowCpCoeffs"].split()[3]) - 5.641515e-09) < 1e-12
    ...
    assert abs(float(species_prop["water"]["gas"]["thermodynamics"]["highCpCoeffs"].split()[1]) - 0.00217691804) < 1e-12
...
def test_species_key_pair():
    ...
    liq_spec_names = get_species_name(case_dir, phase="liquid")
    thermo_file = os.path.join(case_dir, "constant", "thermophysicalProperties.liquid")
