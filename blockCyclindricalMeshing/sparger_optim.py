...
def side_sparger_variations(nCases, study_folder, template_folder='template_sideSparger'):
    ...
    for i in range(nCases):
        ...
        # Setup json files
        modify_sideSparger(heights[i], template_folder, os.path.join(case_folder, 'system'))
        # Generate blockmesh
...
def flat_donut_variations(nCases, study_folder, template_folder='template_flatDonut'):
    ...
    for i in range(nCases):
        ...
        setupCaseFolder(case_folder)
        # Setup json files
        modify_flatDonut(widths[i], template_folder, os.path.join(case_folder, 'system'))
        # Generate blockmesh
        generate_blockMeshDict(os.path.join(case_folder))
        
def multiRing_variations(nCases, study_folder, template_folder='template_multiRing'):
    os.makedirs(study_folder, exist_ok=True)   
    ...
    for i in range(nCases):
        ...
        setupCaseFolder(case_folder)
        # Setup json files
        modify_multiring(widths[i], spacings[i], template_folder, os.path.join(case_folder, 'system'))
        # Generate blockmesh
        generate_blockMeshDict(os.path.join(case_folder))
