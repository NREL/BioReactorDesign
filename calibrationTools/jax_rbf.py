...
def rbf_interpolation_jax(x_data, y, x_interp, length_scale=1.0, amplitude=1.0):
    ...
    z_interp = jnp.einsum(
        "ij,j->i", rbf_kernel(x_interp, x_data, length_scale), weights
    )
    return z_interp
...
if __name__ == "__main__":
    ...
    if ndim == 1:
        ...
        kernel = 1 * RBF(
            length_scale=[1.0] * ndim, length_scale_bounds=[(1e-2, 1e2)] * ndim
        )
        ...
        mean_pred_jax = rbf_interpolation_jax(
            X,
            np.squeeze(y),
            X_interp,
            length_scale=gp_params["k2__length_scale"],
            amplitude=gp_params["k1__constant_value"],
        )
