...
scale = DefineNumber[ 0.05, Name "Parameters/scale"];
...
// Circle (not used)
Point(1) = {brad*2 + lx, -rad, 0, 0.1*scale};
Point(2) = {brad*2 + lx,  0, 0, 0.1*scale};
Point(3) = {brad*2 + lx,  rad, 0, 0.1*scale};

// Path

Point(4) = {brad*2 + lx, 0, lz/2, 0.1*scale};
Point(5) = {brad + lx, 0, lz/2 + brad, 0.1*scale};
Point(6) = {brad, 0, lz/2 + brad, 0.1*scale};
Point(7) = {0, brad, lz/2 + brad, 0.1*scale};
Point(8) = {0, ly + brad, lz/2 + brad, 0.1*scale};
Point(9) = {0, ly + 2*brad, lz/2 , 0.1*scale};
Point(10) = {0, ly + 2*brad, 0 , 0.1*scale};

// Curvature Centers
Point(11) = {lx + brad , 0, lz/2, 0.1*scale};
Point(12) = {brad, brad, lz/2 + brad, 0.1*scale };
Point(13) = {0, ly + brad, lz/2, 0.1*scale};
...
//+ Inlets
Cylinder(10) = {lx/2.0, -rad - rad/2.5, lz  , 0, rad, 0, rad/2.5, 2*Pi};
Cylinder(11) = {lx/2.0, -rad - rad/2.5, -lz , 0, rad, 0, rad/2.5, 2*Pi};
Cylinder(12) = {lx + 2*brad, -rad - rad/2.5, 0 , 0, rad, 0, rad/2.5, 2*Pi};

//+ Outlets
Cylinder(100) = {0, ly + 2*brad,  lz/2.0, 0, 2*rad, 0, rad/2.5, 2*Pi};
Cylinder(101) = {0, ly + 2*brad,  -lz/2.0, 0, 2*rad, 0, rad/2.5, 2*Pi};
...
Mesh.MeshSizeMin = 0.05*scale;
Mesh.MeshSizeMax = 0.1*scale;
