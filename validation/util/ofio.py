...
def readSizeGroups(file):
    ...
    for igroup, group in enumerate(groups):
        if igroup==0:
           bin_size = sizeGroup[groups[igroup+1]] - sizeGroup[groups[igroup]]
        elif igroup==len(groups)-1:
           bin_size = sizeGroup[groups[igroup]] - sizeGroup[groups[igroup-1]]
        else:
           bin_size_p = sizeGroup[groups[igroup+1]] - sizeGroup[groups[igroup]]
           bin_size_m = sizeGroup[groups[igroup]] - sizeGroup[groups[igroup-1]]
...
def conditionalAverage(x, y, nbin):
    ...
    x_bin = np.linspace(np.amin(x) - mag/(2*nbin), np.amax(x) + mag/(2*nbin), nbin)
    ...
    for i in range(nbin):
        ...
        avalsum[i] = np.sum(a[np.argwhere(inds==i)] * y[np.argwhere(inds==i)]) 
        bvalsum[i] = np.sum(b[np.argwhere(inds==i+1)] * y[np.argwhere(inds==i+1)])
    weight = asum + bsum
