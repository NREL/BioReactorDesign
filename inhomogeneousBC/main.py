...
def writeFfield(filename, fieldname, xcent, zcent, schedule, defaultVal, ind):
    ...
    for bound in cmBound:
        ...
        fw.write( '            {\n')
        fw.write(f'                scalar xpos = boundaryPatch.Cf()[faceI][0]-{xcent};\n')
        fw.write(f'                scalar zpos = boundaryPatch.Cf()[faceI][2]-{zcent};\n')
        fw.write( '                scalar rad = std::sqrt(xpos*xpos + zpos*zpos);\n')
        fw.write(f'                if( rad < {schedule["r_const"]} )\n')
        fw.write( '                {\n')
        fw.write(f'                    this->refValue()[faceI] = {schedule["f_in"][ind]};\n')
        fw.write( '                    this->refGrad()[faceI] = 0.0;\n')
        fw.write( '                    this->valueFraction()[faceI] = 1.0;\n')
        fw.write(f'                }} else if (rad < {schedule["r_end"]}){{\n')
        fw.write(f'                    this->refValue()[faceI] = {schedule["f_in"][ind]} + (rad - {schedule["r_const"]})/({schedule["r_end"] - schedule["r_const"]}) * ({schedule["f_out"][ind] - schedule["f_in"][ind]});\n')
        fw.write( '                    this->refGrad()[faceI] = 0.0;\n')
        fw.write( '                    this->valueFraction()[faceI] = 1.0;\n')
        fw.write( '                } else {\n')
        fw.write(f'                    this->refValue()[faceI] = {schedule["f_out"][ind]};\n')
        fw.write( '                    this->refGrad()[faceI] = 0.0;\n')
        fw.write( '                    this->valueFraction()[faceI] = 1.0;\n')
        fw.write( '                }\n')
        fw.write( '            }\n')
