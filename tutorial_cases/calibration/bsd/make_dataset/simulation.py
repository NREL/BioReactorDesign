...
class Simulation:
    ...
    def run(
        ...
        window_size: int = 100,
        mean_tol: float = 1e-8,
        std_tol: float = np.inf,
        kill_failed_sim: bool = False,
        x_pdf: None | np.ndarray = None,
        xlen: int = 100,
    ) -> dict:
        print_progress_bar(
            0,
        ...
        for i_t in range(1, self.nt + 1):
            ...
            if kill_failed_sim and (
                n_bubbles > (init_no_bubbles * 10)
                or n_bubbles < (init_no_bubbles * 0.1)
            ):
                return {}
            # Check when we are in statistically stationary region
            if not start_ave:
                converged = check_conv(
                    mean_diameter_history, window_size, mean_tol, std_tol
                )
                if converged:
                    start_ave = True
        ...
        if not start_ave:
            return {}
        result_dict = {
            "mean_diameter_history": np.array(mean_diameter_history),
            "y_pdf": np.array(y_pdf),
            "x_pdf": np.array(x_pdf),
            "start_ave_time": start_ave_time,
        }
        return result_dict
