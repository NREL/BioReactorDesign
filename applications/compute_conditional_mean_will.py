...
def compute_radius_field(cellCentres: np.ndarray, vert_ind: int) -> np.ndarray:
    """
    Compute radius from cell centres. The radius goes negative to make it look like brooks and chen
    """
    if vert_ind == 0:
        radius = np.sqrt(cellCentres[:, 1] ** 2 + cellCentres[:, 2] ** 2)
        ind_min = np.argwhere(cellCentres[:, 1] < 0)
        radius[ind_min] = -1 * radius[ind_min]
    elif vert_ind == 1:
        radius = np.sqrt(cellCentres[:, 0] ** 2 + cellCentres[:, 2] ** 2)
        ind_min = np.argwhere(cellCentres[:, 0] < 0)
        radius[ind_min] = -1 * radius[ind_min]
    elif vert_ind == 2:
        radius = np.sqrt(cellCentres[:, 0] ** 2 + cellCentres[:, 1] ** 2)
        ind_min = np.argwhere(cellCentres[:, 0] < 0)
        radius[ind_min] = -1 * radius[ind_min]
    else:
...
def get_heights_ind(
    ...
    if tolerance is None:
        A = np.unique(cellCentres[:, vert_ind])
        n_vert = len(A)
    ...
    for height in heights:
        ...
        ind_heights[height2str(height)] = np.argwhere(
            abs(cellCentres[:, vert_ind] - height) <= tolerance
        )
...
def radial_mean(
    ...
    logger.info(f"Case : {caseFolder}")

    # Get all time folders
    time_float_sorted, time_str_sorted = getCaseTimes(caseFolder)

    # Get cell centers
    mesh_time_str = getMeshTime(caseFolder)
    cellCentres = readMesh(
        os.path.join(caseFolder, f"meshCellCentres_{mesh_time_str}.obj")
    )
    ...
    radius = compute_radius_field(cellCentres, vert_ind)
    ...
    for i_ave in range(window_ave):
        ...
        for field_name in field_names:
            field_file.append(
                os.path.join(caseFolder, time_folder, field_name)
            )
