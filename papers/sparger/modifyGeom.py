...
def modify_multiring(width, spacing, template_folder, target_folder):
    ...
    with open(os.path.join(target_folder, "input.json"), "w+") as f:
        ...
        data_input["Geometry"]["Radial"]["inner_ring5"] = (
            r_outterRing - ring_width
        )
        data_input["Geometry"]["Radial"]["outter_ring4"] = (
            r_outterRing - ring_width - ring_spacing
        )
        data_input["Geometry"]["Radial"]["inner_ring4"] = (
            r_outterRing - 2 * ring_width - ring_spacing
        )
