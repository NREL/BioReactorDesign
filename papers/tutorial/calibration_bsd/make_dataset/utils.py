...
def check_conv(values: list[float]|np.ndarray, window_ave: int=5, mean_threshold: float=1e-6, std_threshold: float=1e-6) -> bool:
    """
    ...
    window_avgs = np.convolve(np.array(values), np.ones(window_ave), 'valid')/window_ave
   
    # Compare means and standard deviation of the last two window averages
    mean_diff = abs(np.mean(window_avgs[-2:]) - np.mean(window_avgs[-window_ave:]))
    std_diff = abs(np.std(window_avgs[-2:]) - np.std(window_avgs[-window_ave:]))
    #print(window_avgs)
