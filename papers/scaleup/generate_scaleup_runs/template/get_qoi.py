...
def get_qoi_uq(kla_co2, cs_co2, kla_h2, cs_h2):
    ...
    for i in range(len(kla_co2)):
        if i == 0:
            ...
            verb = False
        qoi_tmp, qoi_kla_tmp, qoi_sum_tmp, qoi_sum_kla_tmp = get_qoi(
            kla_co2[i], cs_co2[i], kla_h2[i], cs_h2[i], verb
        )
        qoi.append(qoi_tmp)
        qoi_kla.append(qoi_kla_tmp)
        qoi_sum.append(qoi_sum_tmp)
        qoi_sum_kla.append(qoi_sum_kla_tmp)
    qoi = np.array(qoi)
    qoi_kla = np.array(qoi_kla)
    qoi_sum = np.array(qoi_sum)
    qoi_sum_kla = np.array(qoi_sum_kla)
    return (
        np.mean(qoi),
        np.std(qoi),
        np.mean(qoi_kla),
...
(
    ...
    qoi_sum_kla_m,
    qoi_sum_kla_s,
) = get_qoi_uq(tmp_kla_co2, tmp_cs_co2, tmp_kla_h2, tmp_cs_h2)
...
with open("qoi_sum.txt", "w+") as f:
    f.write(f"{qoi_sum_m},{qoi_sum_s}\n")
with open("qoi_sum_kla.txt", "w+") as f:
    f.write(f"{qoi_sum_kla_m},{qoi_sum_kla_s}\n")
