...
def load_observation_data():
    if valexp.lower() == "co2":
        A = np.load('../../validation/refData/data/val2_data.npz')[f'xco2_exp{nexp}'].astype('float32')
    elif valexp.lower() == "gh":
        A = np.load('../../validation/refData/data/val2_data.npz')[f'gh_exp{nexp}'].astype('float32')
    data_z = A[:,1]
    data_y = A[:,0]
...
nn = BCR_NN(**nn_kwargs, model_folder=model_folder, weight_file=os.path.join(model_folder,'best.h5'))
scale_z_min = joblib.load(os.path.join(nn.model_folder,"scaler_z.mod")).data_min_
scale_z_max = joblib.load(os.path.join(nn.model_folder,"scaler_z.mod")).data_max_
scale_par_min = joblib.load(os.path.join(nn.model_folder,"scaler_par.mod")).data_min_
scale_par_max = joblib.load(os.path.join(nn.model_folder,"scaler_par.mod")).data_max_
scale_y_mean = joblib.load(os.path.join(nn.model_folder,"scaler_y.mod")).mean_
scale_y_scale = joblib.load(os.path.join(nn.model_folder,"scaler_y.mod")).scale_
...
def bayes_step(y=None, y_err=0.1):
    ...
    y_model, _ = jax_func(jax_params, jnp.array([surfaceTension, coal_eff, breakup_eff]))
...
def mcmc_iter(y_err=0.1, mcmc_method="HMC"):
    ...
    for i in range(nsamples):
        y = forward(np_mcmc_samples[i, :].astype('float32'))
    ...
    if np.amax(true_m95  - min_real) > 0 or np.amin(true_p95 - max_real) < 0:
        #fig=plt.figure()
        #plt.plot(true_m95, data_z, 'o', color='r')
        #plt.plot(min_real, data_z, color='k')
        #plt.plot(true_p95, data_z, 'o', color='r')
        #plt.plot(max_real, data_z, color='k')
        #plt.show()
        print(f" Increase STD  {np.amax(true_m95 - min_real)} - {np.amin(true_p95 - max_real)}")
...
# Uncertainty propagation
rangez = np.reshape(np.linspace(np.amin(data_z), np.amax(data_z), 250), (250,1))
...
plt.plot(data_y + 2*sigma, data_z, '--', color='r', label="95% Exp. confidence interval")
#ax.grid()
plt.plot(mean_real, rangez, color="k", linewidth=3, label="mean degradation")
plt.plot(
    std97_5_real, rangez, "--", color="k", linewidth=3, label="95% Model confidence interval"
)
plt.plot(std2_5_real, rangez, "--", color="k", linewidth=3)
pretty_labels(xLabel, yLabel, 20, title=f"Exp uncertainty = {sigma:.4g}", fontname="Times New Roman")
#plotLegend()
