...
err_list = [sigmas["17_co2"], sigmas["19_co2"], sigmas["17_gh"], sigmas["19_gh"]]

nn_kwargs = {"input_dim": 4, "output_dim": 1, "units": [10, 20, 10, 5], "activation": "tanh"}
...
z_tens_list = [tf.convert_to_tensor(z, dtype=tf.dtypes.float32) for z in z_list]
ones_tf32_list = [tf.ones(tf.shape(z_tens[:,0]), dtype=tf.dtypes.float32) for z_tens in z_tens_list]
nn_list = [BCR_NN(**nn_kwargs, model_folder=mod, weight_file=os.path.join(mod,'best.h5')) for mod in mod_list]
scale_z_min_list = [joblib.load(os.path.join(nn.model_folder,"scaler_z.mod")).data_min_ for nn in nn_list]
scale_z_max_list = [joblib.load(os.path.join(nn.model_folder,"scaler_z.mod")).data_max_ for nn in nn_list]
scale_par_min_list = [joblib.load(os.path.join(nn.model_folder,"scaler_par.mod")).data_min_ for nn in nn_list]
scale_par_max_list = [joblib.load(os.path.join(nn.model_folder,"scaler_par.mod")).data_max_ for nn in nn_list]
scale_y_mean_list = [joblib.load(os.path.join(nn.model_folder,"scaler_y.mod")).mean_ for nn in nn_list]
scale_y_scale_list = [joblib.load(os.path.join(nn.model_folder,"scaler_y.mod")).scale_ for nn in nn_list]


@tf.function
def forward(p):
  par_tens_list = [tf.stack([p[i] * ones_tf32 for i in range(len(p))], axis=1) for ones_tf32 in ones_tf32_list]
  par_tens_resc_list = [scale_par(par_tens, scale_par_min, scale_par_max) for (par_tens, scale_par_min, scale_par_max) in zip(par_tens_list, scale_par_min_list, scale_par_max_list)]
  z_tens_resc_list = [scale_z(z_tens, scale_z_min, scale_z_max) for (z_tens, scale_z_min, scale_z_max) in zip(z_tens_list, scale_z_min_list, scale_z_max_list)]
  out_resc_list = [nn.model([z_tens_resc, par_tens_resc]) for (nn, z_tens_resc, par_tens_resc) in zip(nn_list, z_tens_resc_list, par_tens_resc_list)]
  out_list = [unscale_y(out_resc, scale_y_mean, scale_y_scale) for (out_resc, scale_y_mean, scale_y_scale) in zip(out_resc_list, scale_y_mean_list, scale_y_scale_list)]
...
def bayes_step(y_list=None, y_err_list=[0.1]):
    surfaceTension = numpyro.sample("surfaceTension", dist.Uniform(0.065, 0.075))
    coal_eff = numpyro.sample("coal_eff", dist.Uniform(0.05, 20))
    breakup_eff = numpyro.sample("breakup_eff", dist.Uniform(0.05, 20))
    y_model_list, _ = jax_func(jax_params, jnp.array([surfaceTension, coal_eff, breakup_eff]))
    std_obs = jnp.concatenate([jnp.ones(y_model.shape[0])*y_err for (y_model, y_err) in zip(y_model_list, y_err_list)])
    numpyro.sample("obs", dist.Normal(jnp.concatenate(y_model_list, axis=0), std_obs), obs=jnp.concatenate(y_list, axis=0))
...
def mcmc_iter(mcmc_method="HMC"):
    ...
    mcmc = MCMC(kernel, num_chains=1, num_warmup=num_warmup, num_samples=num_samples, jit_model_args=True)
    ...
    for ilabel, label in enumerate(labels):
        for ipar, name in enumerate(['surfaceTension', 'coal_eff', 'breakup_eff']):
            if label == name:
                nplabel = labels_np.index(name)
        np_mcmc_samples[:, nplabel] = np.array(mcmc_samples[label])

    results = {"samples": np_mcmc_samples, "labels_np": labels_np, "labels": labels}
...
np.savez(os.path.join("samp.npz"), samples=np_mcmc_samples, labels_np=labels_np, labels=labels)
...
fig = corner.corner(np_mcmc_samples, truths=truths, labels=labels_np_disp, truth_color='k', bins=50, range=ranges)
for ax in fig.get_axes():
    ax.tick_params(axis='both', labelsize=20)  # Customize font size, line width, and tick length
...
rangez = np.reshape(np.linspace(np.amin(np.concatenate(data_z_list, axis=0)), np.amax(np.concatenate(data_z_list, axis=0)), 250), (250,1))
...
def forward_range(p, rangez):
  ...
  par_resc_list = [scale_par(par, scale_par_min, scale_par_max) for (scale_par_min, scale_par_max) in zip(scale_par_min_list, scale_par_max_list)]
  z_tens_resc_list = [scale_z(z_tens, scale_z_min, scale_z_max) for (scale_z_min, scale_z_max) in zip(scale_z_min_list, scale_z_max_list)]
  out_resc_list = [nn.model([z_tens_resc, par_resc]) for (nn, z_tens_resc, par_resc) in zip(nn_list, z_tens_resc_list, par_resc_list)]
  out_list = [unscale_y(out_resc, scale_y_mean, scale_y_scale) for (out_resc, scale_y_mean, scale_y_scale) in zip(out_resc_list, scale_y_mean_list, scale_y_scale_list)]
  return [out[:,0] for out in out_list]
...
for i in range(nsamples):
    y = forward_range(np_mcmc_samples[i, :].astype('float32'), rangez.astype('float32'))
    for iobs, obs_name in enumerate(obs_name_list):
        realization_list[iobs].append(y[iobs])
for iobs, obs_name in enumerate(obs_name_list):
    realization_list[iobs] = np.array(realization_list[iobs])

mean_real_list = [np.mean(realization, axis=0) for realization in realization_list]
min_real_list = [np.min(realization, axis=0) for realization in realization_list]
max_real_list = [np.max(realization, axis=0) for realization in realization_list]
std97_5_real_list = [np.percentile(realization, 97.5, axis=0) for realization in realization_list]
std2_5_real_list = [np.percentile(realization, 2.5, axis=0) for realization in realization_list]


for (data_y, data_z, mean_real, std97_5_real, std2_5_real, err, xLabel, obs_name) in zip(data_y_list, data_z_list, mean_real_list, std97_5_real_list, std2_5_real_list, err_list, xLabel_list, obs_name_list):
    fig = plt.figure()
    plt.plot(data_y, data_z, "o", color="r", markersize=7, label="Data")
    plt.plot(data_y + 2*err, data_z, "--", color="r", label="95% Exp confidence")
    plt.plot(data_y - 2*err, data_z, "--", color="r")
    plt.plot(mean_real, rangez, color="k", linewidth=3, label="mean degradation")
    plt.plot(
        std97_5_real, rangez, "--", color="k", linewidth=3, label="95% Model confidence"
    )
