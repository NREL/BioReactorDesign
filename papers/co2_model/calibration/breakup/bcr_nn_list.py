...
def makeModel(input_dim_par, output_dim, units, activation):
    # inputs
    input_z = layers.Input(shape=(1,), name="input_z")
    input_par = layers.Input(shape=(input_dim_par,), name="input_par")
    input_z_par = layers.concatenate(
        [input_z, input_par], name="input_z_par"
    )  
    x = input_z_par
    for unit in units:
...
class BCR_NN(keras.Model):
	...
	input_dim=None,
        ...
        **kwargs,
        ...
        self.model = makeModel(
            self.input_dim_par, self.output_dim, units, activation
        )
    ...
    def make_data(self, np_file=None, scaler_folder='.'):
        ...
        # Split and shuffle
        z_train, z_test, par_train, par_test, y_train, y_test = train_test_split(scaler_z.transform(z_dat), scaler_par.transform(par_dat), scaler_y.transform(y_dat), test_size=0.01, random_state=42)
	...
	data_file = None,
        ...
        dataset = self.make_data(scaler_folder=self.model_folder, np_file=data_file)
        self.n_batch = dataset["z_train"].shape[0] // batch_size
        self.freq = self.n_batch * 10
        # self.freq = 1

        train_dataset = tf.data.Dataset.from_tensor_slices(
            (dataset["z_train"], dataset["par_train"], dataset["y_train"], )
        ).batch(batch_size)
        test_dataset = tf.data.Dataset.from_tensor_slices(
            (dataset["z_test"], dataset["par_test"], dataset["y_test"], )
        ).batch(100000)
        ...
        for epoch in range(nEpochs):
            ...
            for step, batch in enumerate(train_dataset):
                ...
                time_s = time.time()
                loss_info = self.train_step(input_z=batch[0], input_par=batch[1], output_true=batch[2])
            ...
            for val_step, val_batch in enumerate(test_dataset):
                val_loss = self.calc_loss(val_batch[0], val_batch[1], val_batch[2])
                test_loss = (
            ...
            self.model.save_weights(
                os.path.join(self.model_folder, "last.h5"),
                overwrite=True,
            )

    def pred(self, z, par, rescaled=False):
        scaler_z = joblib.load(os.path.join(self.model_folder, "scaler_z.mod"))
        scaler_par = joblib.load(os.path.join(self.model_folder, "scaler_par.mod"))
    ...
    def logTraining(
        self, epoch, loss, val_loss, bestLoss
    ):
        f = open(os.path.join(self.log_loss_folder, "log.csv"), "a+")
        f.write(
            str(int(epoch))
        ...
        if bestLoss is None or loss < bestLoss:
            bestLoss = loss
            self.model.save_weights(
                os.path.join(self.model_folder, "best.h5"), overwrite=True
            )
        return bestLoss

    def get_loss_dat(self):
        lossData = np.genfromtxt(os.path.join(self.log_loss_folder, 'log.csv'), delimiter=";", skip_header=1)
        return {'epoch': lossData[:,0], 'step': lossData[:,1], 'train_loss': lossData[:,2], 'val_loss': lossData[:,3]} 


if __name__ == "__main__":
    myNN = BCR_NN(input_dim=4,
                  output_dim=1,
                  units=[10, 20, 10, 5],
                  activation='tanh',
                  model_folder='Modeltmp',
                  #weight_file='Modeltmp.save/last.h5',
                  log_loss_folder='Logtmp')

    myNN.train(learningRateModel=1e-3, batch_size=32, nEpochs=1000, data_file='train_data_gh_17_raw.npz')
    ...
    fig = plt.figure()
    plt.plot(loss_dat['epoch'], loss_dat['train_loss'], color='k', linewidth=3, label='train')
    plt.plot(loss_dat['epoch'], loss_dat['val_loss'], color='b', linewidth=3, label='test')
